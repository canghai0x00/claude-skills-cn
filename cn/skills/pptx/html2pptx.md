...
使用 `html2pptx.js` 库将 HTML 幻灯片转换为 PowerPoint 演示文稿，并实现精确定位。
...
- 内联格式：使用 `<b>`、`<i>`、`<u>` 标签或带 CSS 样式的 `<span>`
  ...
  - `<span>` 不支持：`margin`、`padding`（PowerPoint 文本段落不支持）
  - 示例：`<span style="font-weight: bold; color: #667eea;">粗体蓝色文本</span>`
- Flexbox 有效 - 位置根据渲染布局计算
- 在 CSS 中使用带 `#` 前缀的十六进制颜色
- **文本对齐**：需要时使用 CSS `text-align`（`center`、`right` 等）作为 PptxGenJS 文本格式的提示，以防文本长度略有偏差

### 形状样式（仅限 DIV 元素）

**重要提示：背景、边框和阴影仅在 `<div>` 元素上生效，不适用于文本元素（`<p>`、`<h1>`-`<h6>`、`<ul>`、`<ol>`）**
...
- **重要提示：切勿使用 CSS 渐变（`linear-gradient`、`radial-gradient`）** - 它们不会转换为 PowerPoint
- **务必先使用 Sharp 创建渐变/图标 PNG，然后在 HTML 中引用**
- 对于渐变：将 SVG 转换为 PNG 背景图像
- 对于图标：将 react-icons SVG 转换为 PNG 图像
- 所有视觉效果必须在 HTML 渲染前预先渲染为光栅图像
...
2. **内容不得溢出正文** - 报告具体测量的溢出
3. **CSS 渐变** - 报告不支持的渐变使用
4. **文本元素样式** - 报告文本元素上的背景/边框/阴影（仅允许在 div 上使用）

**所有验证错误都会一起收集并报告**在单个错误消息中，让您能够一次性修复所有问题，而不是一次修复一个问题。
...
- **切勿在 PptxGenJS 中使用带有 `#` 前缀的十六进制颜色** - 会导致文件损坏
...
slide.addChart(
  ...
  ],
  {
    ...
    showValAxisTitle: true,
    valAxisTitle: "销售额 (千美元)",
    // 可选：控制缩放（根据数据范围调整最小值，以便更好地可视化）
    valAxisMaxVal: 8000,
    valAxisMinVal: 0, // 对于计数/金额使用 0；对于聚集数据（例如 4500-7100），考虑从接近最小值开始
    valAxisMajorUnit: 2000, // 控制 y 轴标签间距，防止拥挤
    catAxisLabelRotate: 45, // 如果拥挤，旋转标签
    dataLabelPosition: "outEnd",
    dataLabelColor: "000000",
    // 为单系列图表使用单一颜色
    chartColors: ["4472C4"], // 所有条形图同一颜色
  }
);
```
...
**重要提示**：散点图数据格式特殊 - 第一个系列包含 X 轴值，后续系列包含 Y 值：
...
slide.addChart(
  ...
  ],
  {
    ...
    showValAxisTitle: true,
    valAxisTitle: "温度 (°F)",
    // 可选：Y 轴范围（根据数据范围设置最小值，以便更好地可视化）
    valAxisMinVal: 0, // 对于从 0 开始的范围（计数、百分比等）
    valAxisMaxVal: 60,
    valAxisMajorUnit: 20, // 控制 y 轴标签间距，防止拥挤（例如 10, 20, 25）
    // valAxisMinVal: 30,  // 首选：对于在一定范围内聚集的数据（例如 32-55 或 3-5 的评分），从接近最小值的地方开始轴，以显示变化
    // 可选：图表颜色
...
**重要提示**：饼图需要**单个数据系列**，其中所有类别都在 `labels` 数组中，相应的值在 `values` 数组中。
...
**重要提示**：使用**不带** `#` 前缀的十六进制颜色 - 包含 `#` 会导致文件损坏。

**将图表颜色与您选择的设计调色板对齐**，确保数据可视化时有足够的对比度和区分度。调整颜色以实现：
