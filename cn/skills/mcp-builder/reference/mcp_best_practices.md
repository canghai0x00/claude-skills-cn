...
名称应该是通用的，描述所集成的服务，容易从任务描述中推断出来，且不包含版本号。
...
**注意**：stdio 服务器不应该记录到 stdout（使用 stderr 进行日志记录）
...
| `idempotentHint`  | 布尔值 | false  | 使用相同参数的重复调用没有额外效果 |
...
try {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: `错误：${error.message}。尝试使用 filter='active_only' 减少结果。`,
      },
    ],
  };
}
