**重要提示：您必须按顺序完成这些步骤。不要跳过步骤直接编写代码。**

如果您需要填写 PDF 表单，首先检查该 PDF 是否有可填写的表单字段。从本文件所在目录运行以下脚本：
`python scripts/check_fillable_fields <file.pdf>`，根据结果转到"可填写字段"或"不可填写字段"部分并按照相应说明操作。
...
[
  {
    "field_id": (字段的唯一ID),
    "page": (页码，从1开始),
    "rect": ([左, 下, 右, 上]在PDF坐标中的边界框，y=0是页面底部),
...
[
  ...
  },
  {
    ...
    "page": 1,
    "value": "/On" // 如果这是一个复选框，使用其"checked_value"值来选中它。如果是单选按钮组，使用"radio_options"中的某个"value"值。
  },
  // 更多字段
]
```

- 从本文件所在目录运行`fill_fillable_fields.py`脚本创建填好的 PDF：
  `python scripts/fill_fillable_fields.py <input pdf> <field_values.json> <output pdf>`
  该脚本将验证您提供的字段 ID 和值是否有效；如果打印错误消息，请更正相应的字段并重试。

# 不可填写字段

如果 PDF 没有可填写的表单字段，您需要通过视觉确定应添加数据的位置并创建文本注释。请*准确*按照以下步骤操作。您必须执行所有这些步骤以确保表单准确完成。每个步骤的详细信息如下。

- 将 PDF 转换为 PNG 图像并确定字段边界框。
- 创建一个包含字段信息的 JSON 文件，以及显示边界框的验证图像。
- 验证边界框。
- 使用边界框填写表单。
...
- 仔细检查每个 PNG 图像并识别所有表单字段和用户应输入数据的区域。对于用户应输入文本的每个表单字段，确定表单字段标签和用户应输入文本的区域的边界框。标签和输入边界框不得相交；文本输入框应只包括应输入数据的区域。通常，此区域将紧靠在其标签的侧面、上方或下方。输入边界框必须足够高和宽以容纳其文本。
...
输入区域应位于线的上方，并包括线的整个宽度。这常见于签名和日期字段。
...
- 寻找小方框(□) - 这些是要定位的实际复选框。它们可能在其标签的左边或右边。
- 区分标签文本("是"、"否")和可点击的复选框方块。
- 输入边界框应只覆盖小方块，而不是文本标签。

### 步骤 2：创建 fields.json 和验证图像（必需）

- 创建一个名为`fields.json`的文件，其中包含以下格式的表单字段和边界框信息：
...
```
{
  ...
  "form_fields": [
    ...
    // 复选框示例。对于entry_bounding_box，请定位方块，而不是文本
    {
      "page_number": 2,
...
- 通过使用`check_bounding_boxes.py`脚本检查 fields.json 文件（从本文件所在目录运行），验证没有边界框相交，并且输入边界框足够高：
...
- 如果任何矩形看起来不正确，请修正 fields.json，重新生成验证图像，并再次验证。重复此过程，直到边界框完全准确为止。

### 步骤 4：向 PDF 添加注释

从本文件所在目录运行此脚本，使用 fields.json 中的信息创建填好的 PDF：
