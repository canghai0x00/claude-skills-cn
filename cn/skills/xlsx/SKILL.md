---
name: xlsx
description: "全面的电子表格创建、编辑和分析，支持公式、格式化、数据分析和可视化。当 Claude 需要处理电子表格（.xlsx、.xlsm、.csv、.tsv 等）用于：(1) 创建带有公式和格式的新电子表格，(2) 读取或分析数据，(3) 修改现有电子表格同时保留公式，(4) 在电子表格中进行数据分析和可视化，或 (5) 重新计算公式"
...
- **百分比**：默认为 0.0% 格式（一位小数）
- **倍数**：对估值倍数格式化为 0.0x（EV/EBITDA、P/E）
...
- 注释或在单元格旁边（如果在表格末尾）。格式："来源：[系统/文档]，[日期]，[具体引用]，[URL 如适用]"
- 示例：
...
用户可能会要求您创建、编辑或分析 .xlsx 文件的内容。您有不同的工具和工作流程可用于不同任务。

## 重要要求

**需要 LibreOffice 进行公式重新计算**：您可以假设已安装 LibreOffice 来使用 `recalc.py` 脚本重新计算公式值。该脚本在首次运行时会自动配置 LibreOffice。
...
1. **选择工具**：使用 pandas 进行数据处理，openpyxl 进行公式/格式化
2. **创建/加载**：创建新工作簿或加载现有文件
3. **修改**：添加/编辑数据、公式和格式
4. **保存**：写入文件
5. **重新计算公式（如果使用公式则必须进行）**：使用 recalc.py 脚本
   ```bash
...
- [ ] **测试 2-3 个样本引用**：在构建完整模型之前验证它们是否提取正确值
...
- 单元格索引是从 1 开始的（row=1, column=1 表示单元格 A1）
